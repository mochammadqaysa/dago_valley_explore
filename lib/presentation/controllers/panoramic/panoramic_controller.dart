...
import 'package:dago_valley_explore/app/util/platform_image_helper.dart'
    as img_helper;
import 'package:dago_valley_explore/app/util/platform_image_helper_stub.dart'
    if (dart.library.io) 'package:dago_valley_explore/app/util/platform_image_helper_io.dart'
    as img_impl;
...
class PanoramicController extends GetxController {
  ...
  final _imageFiles = <String, Rx<dynamic>>{}.obs;
  ...
  Future<dynamic> getLocalImage(String imageUrl) async {
    ...
    if (!_imageLoadingStates.containsKey(imageUrl)) {
      ...
      _imageFiles[imageUrl] = Rx<dynamic>(null);
    }
  ...
  dynamic getCachedImageFile(String imageUrl) {
  ...
  // Helper method to check if file exists (cross-platform)
  bool fileExists(dynamic file) {
    if (file == null) return false;
    return img_impl.fileExists(file);
  }
}
